...
function FootballSection({ matches, loading, lastUpdate }: { matches: Match[]; loading: boolean; lastUpdate: Date }) {
  ...
  const safes = activeMatches.filter(m => m.insight.riskPercentage <= 40);
  const moderate = activeMatches.filter(m => m.insight.riskPercentage > 40 && m.insight.riskPercentage <= 55);
  const risky = activeMatches.filter(m => m.insight.riskPercentage > 55);
...
function BasketballSection({ matches, loading, lastUpdate }: { matches: Match[]; loading: boolean; lastUpdate: Date }) {
  ...
  const finishedMatches = matches.filter(m => m.isFinished);
  const upcomingMatches = matches.filter(m => !m.isLive && !m.isFinished);
  const safes = upcomingMatches.filter(m => m.insight.riskPercentage <= 45);
...
function AppDashboard({ onLogout, userInfo }: { onLogout: () => void; userInfo: UserInfo | null }) {
  ...
  const [activeSection, setActiveSection] = useState<'football' | 'basketball' | 'nhl' | 'nfl' | 'tennis' | 'expert' | 'analyse' | 'antitrap' | 'bankroll' | 'results' | 'pronostiqueur' | 'admin'>('football');
  ...
  const activeMatches = [...upcomingMatches, ...liveMatches];
  const safes = activeMatches.filter(m => m.insight.riskPercentage <= 40);
  const moderate = activeMatches.filter(m => m.insight.riskPercentage > 40 && m.insight.riskPercentage <= 50);
  const risky = activeMatches.filter(m => m.insight.riskPercentage > 50);
  const valueBets = activeMatches.filter(m => m.insight.valueBetDetected);
