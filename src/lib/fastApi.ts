...
export async function getFastMatches(): Promise<{
  ...
  try {
    ...
    const [nbaMatches, footballMatches, nflMatches, nhlMatches] = await withTimeout(
      Promise.all([fastNBAMatches(), fastFootballMatches(), fastNFLMatches(), fastNHLMatches()]),
      GLOBAL_TIMEOUT
    );
    
    const allMatches = [...footballMatches, ...nbaMatches, ...nflMatches, ...nhlMatches];
    ...
    return {
      matches: [...cachedFootball, ...cachedNBA, ...cachedNFL, ...cachedNHL],
      timing: {
